...
LastEditTime : 2021-06-30 14:54:23
'''
...
def plot_sharding():
    ...
    plt.errorbar(times, mean1_size/1000, yerr=stdv1_size/1000,
                 linewidth=0.6, marker="o", markersize=5, elinewidth=0.6)
    plt.errorbar(times, mean2_size/1000, yerr=stdv2_size/1000,
                 linewidth=0.6, marker="^", markersize=5, elinewidth=0.6)
    plt.errorbar(times, mean3_size/1000, yerr=stdv3_size/1000,
                 linewidth=0.6, marker="P", markersize=5, elinewidth=0.6)
    plt.errorbar(times, mean4_size/1000, yerr=stdv4_size/1000,
                 linewidth=0.6, marker="*", markersize=5, elinewidth=0.6)
    plt.legend(["Shard 1 by size", "Shard 2 by size",
                "Shard 3 by size", "Shard 4 by size"])
...
def plot_ecdf(data):
    ...
    ax1.plot(ecdf.x, ecdf.y, lw=0.7)
    ax1.set_xscale("log")
    ax1.set_xticks([1000, 1500, 2000, 2500, 3000],
                   ["1e3, 1.5e3, 2e3, 2.5e3, 3e3"])
    ax1.set_xlabel("Query Amount")
    ax1.set_ylabel("Cumulative Probability")
    ax1.set_title("Query Amount CDF of all Shards")
    ax1.grid(True)
    plt.show()
    # fig1.savefig("cdf.pdf", bbox_inches="tight")
